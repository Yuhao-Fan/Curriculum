package net.togogo.servlet;

import net.togogo.bean.Course;
import net.togogo.service.ManageService;
import net.togogo.service.impl.ManageServiceImpl;

import javax.servlet.ServletException;
import javax.servlet.annotation.WebServlet;
import javax.servlet.http.HttpServlet;
import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;
import java.io.IOException;
import java.util.List;

@WebServlet("/GradesGetCourse")
public class GradesGetCourse extends HttpServlet {
    ManageService manageService = new ManageServiceImpl();

    @Override
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {

        System.out.println("进入获取成绩");
        try {
            List<Course> subjectList = manageService.getCourseAll();

            System.out.println(subjectList);

            req.setAttribute("subjectList",subjectList);
        } catch (Exception e) {
            e.printStackTrace();
        }



        req.getRequestDispatcher("gradesAdd.jsp").forward(req,resp);
    }
}
